...
| **integrity pass** | The document-scoped fifth check — one claim per place, one subject class per term, a route reaching every option named, no two passages incompatible — graded against `quill-builder-impl` because it is a relation between passages no scenario reads. |
