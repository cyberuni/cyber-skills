...
| Mark a trend measured across scoring models | the latest and previous records and the scoring model each carries | a trend drawn from two records scored under different models — or from one carrying no model at all — is rendered **marked as cross-model**, naming both models; the mark rides the trend and the needs-attention entry, and changes no health classification |
...
flowchart TD
  ...
  sameModel -- no, or a record carries none --> markModel[mark the trend cross-model, naming both models or unknown]
...
| `sameModel` → record carries none | a suite whose previous record carries no scoring model | `a record carrying no scoring model is marked unknown rather than assumed to match` |
| `markModel` → classification untouched | a suite whose two records were scored under different models | `the cross-model mark does not change how a suite is classified` |
| `markModel` → rides the attention entry | a suite needing attention whose trend is cross-model | `a needs-attention entry drawn from a cross-model pair carries the mark` |
