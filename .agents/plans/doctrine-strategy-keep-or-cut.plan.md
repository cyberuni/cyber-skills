...
todos:
  ...
  - id: queued-ratification-backlog
    status: in_progress
    note: 7 new KEEP CRs (CR-1..CR-7). SHIPPED (PRs open, impl re-verified first-hand): CR-2 cause-enum conformance (also discharged 263-op6-m2 via dimension|clearance|ceiling); CR-1 cold-instrument doctrine (placement escalated → Council Option A: rule 1 on impl-producer, rules 2-3 on doctrine/scanner + spec-producer authoring bars; 13 scenarios/3 nodes); CR-7 ACED untradeable-boolean (placement widened to the spec-validator JUDGE node beyond the brief's two targets — the licensing `only` is judge-side text, so a producer-only CR ships a rule with a green judge still passing the defect; Council ratified as presented. 4 scenarios/2 nodes + the aced-builder-spec bar restructured into two disjoint Selection grounds. Non-vacuity MEASURED by ablation: the pre-CR judge ruled the whole-output dimension a PASS and wrote that the missing boolean twin is what clears it — the CR's premise reproduced verbatim); CR-3 plan-finalize backstop (PR #400 — placed on mission/conductor as the execution-state sibling of the correction-line finalize backstop, NOT handoff; 9 scenarios; Council ruled Option B on the open terminal-`status` question: the enum stays dispatch-flag-only `active | approved` and terminal-ness stays derived, now agreeing with the same ruling made for the Scanner in scanner-stale-plan-status; both gates ratified in-session under auto-none, nothing self-asserted); CR-5 retired-term drift registry (PR #401 — new corpus/retired-terms node + check-retired-terms engine wired into the root check chain; both gates ratified in-session; the seed entry's first run caught 3 live survivors in plugins/aced/readme.md, fixed not allow-listed). Remaining: CR-4 (durable footprint, KEEP narrow — PAUSED at spec gate), CR-6 (shared-primitive sibling followup — BLOCKED at spec gate after two cold-judge rounds; round 2 named two of its blockers as regressions from round 1's own remediation, so the loop stopped for a re-plan rather than a round 3. The keep says "file a followup NAMING THE SIBLINGS", and naming them means computing D, the at-risk sibling set — no formulation of D survived: "nodes that depend on the artifact" collides with ADR-0021's four relations (runtime invocation never touches the spec-graph) and fires on a pure git mv; "nodes whose frozen scenarios assert behavior the artifact realizes" is unbounded, has no index or check, and no scenario tests discovery at all. Second blocker: `derived` is unobservable — the followup line schema carries no origin field, deliberately. Council owes an Option A/B/C ruling on how D is determined before any round 3; see sdd-shared-primitive-sibling-followup.plan.md). Backlog followups logged: check-spec-state doesn't enforce approval.<gate>.cause legality (prose-only); cold-instrument rule-3 self-application (grounded in corpus recurrence = non-author evidence); ACED boolean-form is scoped to UNTAGGED scenarios while CR-7 makes tagged @quality the destination form; no boundary scenario for whole-output-SOUNDING dimension names; the Selection bar's own canonical illustrations (no_npx_dependency, harness_agnostic_and_mcp_free) are LIVE violations in the corpus it polices; cause-enum growth for a skipped producer pre-flight (#398); finalize-vs-checkpoint plan-brief write-path overlap (#399).
---
