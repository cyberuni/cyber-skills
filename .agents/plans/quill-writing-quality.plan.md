...
|---|---|
| **Repetition is not the active ingredient.** Haviland & Clark ran an experiment specifically to rule out a repetition explanation: 1033 ms with repetition vs 1014 ms without, 19 ms *in the wrong direction*. The measured cost — 181 ms — attaches to a passage whose given information has **no retrievable antecedent** (E01, E03). | Replace the recurrence criterion with **unresolvable presupposition**. A claim may recur freely; a passage may not presuppose what the reader cannot retrieve. The symmetrical defect — a **bare cross-reference** that withholds content — is worse, since it guarantees the bridging cost rather than risking it. |
