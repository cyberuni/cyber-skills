...
quill serves its five documentation artifact-types with one squad. `spec-judge: null` degenerates to the SDD default cold judge, `sdd-spec-judge`, which `spec-gate` spawns after resolving the slot. `solution-producer: null` uses the SDD default. Each `null` governance uses the SDD default actor-gate bar.
