...
Loaded by name, never triggered by users. Two faces read it: `quill-spec-writer` reads it forward to self-align while authoring, and `spec-gate` reads it backward to grade — Quill declares `spec-judge: null`, so the gate enforces these criteria itself rather than dispatching a judge agent.
