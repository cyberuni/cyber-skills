...
The bar carries **two instruments**, split by how a verdict is reached, and both run once per document rather than once per scenario. The first defect above is **inspection**: the set is enumerable, the routing is enumerable, and a comparison settles it. The other two are **judgment** — deciding that a term has changed subject class, or that two claims cannot both hold, means reading as a reader, and no comparison settles that. They sit in a **defect catalog** of nine entries, grouped by what the defect does to a reader: what they cannot retrieve (unresolvable presupposition, bare cross-reference, undefined term at first use), what misrepresents what they already have (re-presented as new, term drift, contradiction), and where the document disagrees with its own spec (declaration mismatch, claim without mechanism, orphan claim). Every entry carries a **near-miss** that must not fire, and each group its own citation rule — group A's findings are negatives, so they must show an absence over a named reader path rather than assert one over the document.
