...
3. Drafts the file with `description: "By name only"`, `user-invocable: false`, and `metadata: type: governance` — no `## Why`/`## Rationale` section, atomic and independently falsifiable rules.
4. Creates symlinks per targeted runtime.
5. Runs quality checks (description carries no text beyond the marker, no rationale prose, `Apply when:` scope line) and fixes CRITICAL/HIGH failures before presenting the file.
